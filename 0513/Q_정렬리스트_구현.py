...
for _ in range(20):
   random_numbers.append(random.randint(0,100)) #random.randint 사용하여 정수 난수 생성
...
score_dic = { students:scores for students, scores in zip(students, scores)}
...
sorted_scores = sorted(score_dic.items(), key=lambda item: item[1], reverse=True)
# sorted_scores = sorted(scores, reverse=True)

# 정렬된 튜플 리스트를 다른 딕셔너리에 저장(딕셔너리=java의 map. key(학번):value(점수))
sorted_score_dic = {student: score for student, score in sorted_scores}
...
transformed_score_dic = {i+1: (student, score) for i, (student, score) in enumerate(score_dic.items())}
